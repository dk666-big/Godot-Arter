			...
			(() => {
				...
				pForge.querySelector("#f-batch").addEventListener("click", async () => {
					...
					for (const line of lines) {
						try {
							const url = await callImageGen(line + " , game asset, transparent background, centered", prov, { style: pForge.querySelector("#f-style").value });
							const card = document.createElement("div");
							card.className = "gas-thumb";
				...
				pMap.querySelector("#map-gen").addEventListener("click", async () => {
					...
					try {
						if (mode === "fullmap") {
							...
							const img = await loadImg(await callImageGen(prompt + " , full seamless game map, top-down, high detail, tileable, no UI, no watermark", prov, { size }));
