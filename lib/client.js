  ...
  `);
        ...
        <button class="gas-btn ghost" data-preset-del="${p.id}" style="padding:3px 7px;font-size:10px;color:#e74c3c">删除</button>
			...
			(() => {
				...
				const vxEl = pPreset.querySelector("#phys-vx");
				const vyEl = pPreset.querySelector("#phys-vy");
				...
				function buildPhysicsTSCN() {
					...
					parseFloat(vxEl.value);
					parseFloat(vyEl.value);
					...
					const s2d = {
						capsule: "CollisionShape2D",
						circle: "CollisionShape2D",
						rect: "CollisionShape2D",
						convex: "CollisionShape2D"
					};
					const lines = [
				...
				function buildPhysicsGD() {
					...
					(nameEl.value || "Player").replace(/[^a-zA-Z0-9_]/g, "_");
			...
			(() => {
				...
				function analyzeBitmask() {
					...
					function tileMask(tx, ty) {
						let m = 0;
						const t = getAlpha(tx, ty - 1), tr = getAlpha(tx + 1, ty - 1), r = getAlpha(tx + 1, ty), br = getAlpha(tx + 1, ty + 1), b = getAlpha(tx, ty + 1), bl = getAlpha(tx - 1, ty + 1), l = getAlpha(tx - 1, ty), tl = getAlpha(tx - 1, ty - 1);
						if (t || tr || tl) m |= 1;
						if (tr || r || br) m |= 2;
						if (r || br || l) m |= 4;
						if (br || b || bl) m |= 8;
				...
				exportBtn.addEventListener("click", () => {
					...
					status.textContent = "已导出 TileSet.tres + 源图 PNG（拖入 Godot 即可使用）";
					status.style.color = "var(--ok)";
				});
			})();
			(() => {
				...
				pPost.querySelector("#post-run").addEventListener("click", () => {
					...
					try {
						if (op === "palette") {
							...
							downloadUrl(patchCanvas.toDataURL("image/png"), "9patch_panel.png");
