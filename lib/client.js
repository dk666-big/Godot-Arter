  ...
  `);
			...
			}
			{
				...
				function rebuildStage() {
					stage.innerHTML = "";
					if (!elements.length) {
						stage.innerHTML = "<span class=\"gas-note\">暂无提取元素，先「🔍 自动提取」</span>";
						return;
					}
					const stw = stage.clientWidth || 320, sth = stage.clientHeight || 260;
					elements.forEach((el, i) => {
						const wrap = document.createElement("div");
						wrap.className = "gas-extract-item";
						wrap.dataset.idx = String(i);
						wrap.style.left = Math.min(10 + i * 30, Math.max(0, stw - 90)) + "px";
						wrap.style.top = Math.min(10 + i * 24, Math.max(0, sth - 90)) + "px";
						wrap.style.width = Math.min(110, el.w) + "px";
						wrap.style.height = Math.min(110, el.h) + "px";
				...
				function startDrag(e, wrap, i) {
					...
					const ox = e.clientX - wrap.getBoundingClientRect().left, oy = e.clientY - wrap.getBoundingClientRect().top;
					const move = (ev) => {
						let nx = ev.clientX - st.left - ox, ny = ev.clientY - st.top - oy;
						nx = Math.max(0, Math.min(nx, st.width - wrap.offsetWidth));
						ny = Math.max(0, Math.min(ny, st.height - wrap.offsetHeight));
				...
				async function doAuto() {
					...
					const data = srcCanvas.getContext("2d").getImageData(0, 0, srcCanvas.width, srcCanvas.height).data;
					const T = parseInt(tolEl.value) || 32;
					const minArea = minEl.checked ? Math.max(8, srcCanvas.width * srcCanvas.height * 4e-4) : 0;
				...
				runBtn.addEventListener("click", () => {
					...
					else drawProgress("点选模式：请在上方原图点击要提取的元素");
				});
				...
				stage.addEventListener("mousedown", (e) => {
					if (modeEl.value === "point" && srcCanvas) {}
				});
				resetStage();
				updateSel();
			}
        ...
        <button class="gas-btn ghost" data-preset-del="${p.id}" style="padding:3px 7px;font-size:10px;color:#e74c3c">删除</button>
			...
			function addPresetFromForm() {
				...
				const newItem = {
					...
					name,
					type,
					model: model || pickedModels[0] || "",
					baseUrl,
					apiKey,
					models: pickedModels.slice()
				};
				if (existingId) {
					...
					const worker = async () => {
						while (queue.length && !forgeStop) {
							...
							if (ok) {
								...
								err.textContent = "失败:" + line.slice(0, 20);
