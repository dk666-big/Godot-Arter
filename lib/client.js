  ...
  `);
        ...
        <button class="gas-btn ghost" data-preset-del="${p.id}" style="padding:3px 7px;font-size:10px;color:#e74c3c">删除</button>
			...
			(() => {
				...
				pMap.querySelector("#map-ratio").addEventListener("change", (e) => {
					const val = e.target.value;
					if (val) {
						const [w, h] = val.split("x").map(Number);
						pMap.querySelector("#map-w").value = String(w);
						pMap.querySelector("#map-h").value = String(h);
					}
				});
				pMap.querySelector("#map-gen").addEventListener("click", async () => {
					...
					try {
						if (mode === "fullmap") {
							const mapW = parseInt(pMap.querySelector("#map-w").value) || 2048;
							const mapH = parseInt(pMap.querySelector("#map-h").value) || 2048;
				...
				pMap.querySelector("#map-big")?.addEventListener("click", () => {
					...
					big.width = tilesX * tileSize;
					big.height = tilesY * tileSize;
