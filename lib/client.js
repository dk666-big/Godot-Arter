  ...
  `;
			...
			const loadKeys = () => {
				try {
					...
					side.querySelector("#k-openai").value = j.openai || "";
					side.querySelector("#k-stability").value = j.stability || "";
					side.querySelector("#k-replicate").value = j.replicate || "";
					side.querySelector("#k-sf").value = j.siliconflow || "";
				} catch {}
			};
			const saveKeys = () => {
				const v = {
					openai: side.querySelector("#k-openai").value.trim(),
					stability: side.querySelector("#k-stability").value.trim(),
					replicate: side.querySelector("#k-replicate").value.trim(),
					siliconflow: side.querySelector("#k-sf").value.trim()
				};
				localStorage.setItem(LS, JSON.stringify(v));
				const s = side.querySelector("#keys-status");
				s.style.display = "block";
				setTimeout(() => s.style.display = "none", 1500);
			};
			setTimeout(loadKeys, 0);
			side.querySelector("#save-keys").addEventListener("click", saveKeys);
			side.querySelector("#clear-keys").addEventListener("click", () => {
  ...
  `);
			...
			function switchTab(id) {
				...
				const tips = {
					...
					preset: "API 预设：自定义路由会同步到所有生成面板，Base URL 可只填 /v1，自动补全 images/generations",
			...
			async function addToLibrary(kind, name, url) {
			...
			function providerOptionHtml() {
				return "<option value=\"mock\">本地演示(无Key)</option><option value=\"openai\">OpenAI</option><option value=\"stability\">Stability</option><option value=\"siliconflow\">SiliconFlow</option>" + getCustomProviders().map((p) => `<option value="custom:${p.id}">🔌 ${p.name.replace(/[<>"']/g, "")}</option>`).join("");
			}
			...
			function renderPresetList() {
				...
				if (!presets.length) {
					list.innerHTML = "<span style=\"color:#9aa0a6\">暂无自定义预设 — 在右侧表单中添加第一个</span>";
					return;
				}
        ...
        <button class="gas-btn ghost" data-preset-del="${p.id}" style="padding:3px 7px;font-size:10px;color:#e74c3c">删除</button>
			...
			async function toLocalBlobUrl(url) {
				...
				try {
					...
					if (e && e.name === "AbortError") throw new Error("图片下载超时(30s)：" + url.slice(0, 80));
			...
			async function callImageGen(prompt, provider, opts = {}) {
				...
				if (provider.startsWith("custom:")) {
					...
					if (ref) return await callImageEdits(prompt, resolveEditsEndpoint(endpoint), preset.apiKey, {
						...opts,
						model: preset.model || (preset.type === "siliconflow" ? "black-forest-labs/FLUX.1-schnell" : "dall-e-3")
					});
					const body = {
						prompt: prompt,
						n: 1,
			...
			(() => {
				...
				async function sliceFromFile(file) {
					...
					for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
						...
						fc.width = fw;
						fc.height = fh;
						fc.getContext("2d").drawImage(tmp, c * fw, r * fh, fw, fh, 0, 0, fw, fh);
						frames.push(fc);
				...
				pSheet.querySelector("#s-export").addEventListener("click", () => {
					...
					const mf = {
						...
						frames: frames.map((_, i) => ({
							...
							duration: 1 / (parseInt(fpsEl.value) || 8)
						})),
						godot: {
							type: "SpriteFrames",
							animations: [{
				...
				pSheet.querySelector("#s-save").addEventListener("click", async () => {
					...
					const id = await addToLibrary("spritesheet", "序列帧 " + (/* @__PURE__ */ new Date()).toLocaleTimeString(), packCanvas.toDataURL());
					toast(status, "已入库 " + id);
				});
				pSheet.querySelector("#s-gen").addEventListener("click", async () => {
					...
					try {
						const img = await loadImage(await callImageGen(prompt + " , sprite sheet, transparent background", prov, { size: "1024x512" }));
						const c = document.createElement("canvas");
						c.width = img.width;
						c.height = img.height;
						...
						toast(status, String(e.message), false);
					}
				});
			})();
			(() => {
				...
				pForge.querySelector("#f-batch").addEventListener("click", async () => {
					...
					for (const line of lines) {
						try {
							const url = await callImageGen(line + " , game asset, transparent background, centered", prov, {
								style: pForge.querySelector("#f-style").value,
								reference: refUrl || void 0
							});
							const card = document.createElement("div");
							card.className = "gas-thumb";
							...
							err.textContent = "失败:" + String(e.message).slice(0, 30);
			...
			(() => {
				...
				fileInput.addEventListener("change", async () => {
					const f = fileInput.files?.[0];
					if (!f) return;
					showPreview(await loadImg(URL.createObjectURL(f)), true);
				});
				...
				pMap.querySelector("#map-gen").addEventListener("click", async () => {
					...
					try {
						if (mode === "fullmap") {
							...
							const img = await loadImg(await callImageGen(prompt + " , seamless tileable texture, " + ts + "px", prov, { reference: mapRefUrl || void 0 }));
							showPreview(img, corsSafe);
				...
				pMap.querySelector("#map-wang").addEventListener("click", async () => {
					...
					for (let y = 0; y < 2; y++) for (let x = 0; x < 2; x++) {
						...
						g.fillStyle = "rgba(71,140,191," + (x + y) * .08 + ")";
					...
					updateTiled();
					toast(status, "Wang Tiles 2×2 已生成");
				});
				...
				pMap.querySelector("#map-split").addEventListener("click", () => {
					...
					downloadDataUrl(tsUrl, "tileset_" + Date.now() + ".png");
					downloadDataUrl(jurl, "TileSet.json");
				...
				pMap.querySelector("#map-dl").addEventListener("click", async () => {
					...
					try {
						const r = await fetch(curImg.src);
						if (!r.ok) throw new Error("HTTP " + r.status);
						const blob = await r.blob();
						downloadDataUrl(URL.createObjectURL(blob), "map_" + Date.now() + ".png");
						toast(status, "已下载 PNG", true);
					} catch (e) {
						if (/^https?:/i.test(curImg.src)) {
							window.open(curImg.src, "_blank");
							toast(status, "远程直链无法直接下载，已在新标签打开原图", false);
			...
			(() => {
				...
				function renderGrid() {
					...
					if (!list.length) {
						gridEl.innerHTML = "<div class=\"gas-note\" style=\"grid-column:1/-1\">当前库为空，去各工坊生成后点「📥 入库」即可。</div>";
						return;
					}
					gridEl.innerHTML = "";
					for (const item of list) {
						...
						dl.onclick = () => downloadDataUrl(item.url, item.id + ".png");
				...
				importEl.addEventListener("change", (e) => {
					...
					reader.onload = async () => {
						try {
							...
							await refresh();
							statusEl.textContent = "已导入 " + items.length + " 个素材";
						} catch (err) {
							statusEl.textContent = "导入失败：" + String(err.message || err).slice(0, 60);
						}
					};
					reader.readAsText(f);
				});
				pAsset.querySelector("#al-clear").addEventListener("click", async () => {
					...
					await refresh();
					statusEl.textContent = "已清空 " + LIB_DEFS[activeLib].label;
				});
			})();
