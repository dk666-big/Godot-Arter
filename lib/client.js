...
像素风 木宝箱 关闭/打开 两态"></textarea>
  ...
  `);
			...
			const openWebPanel = async (kind, prompt) => {
				...
				if (prompt) {
					if (await webLinkPushPrompt(kind, prompt)) webToast("🔗 提示词已发送到联动服务 — 装有扩展的浏览器会自动填入 " + (kind === "gemini" ? "Gemini" : "ChatGPT") + " 对话框（不会自动发送）");
					else try {
						await navigator.clipboard.writeText(prompt);
						webToast("📋 联动服务不可用，提示词已复制到剪贴板 — 打开网页后手动粘贴");
					} catch {
						webToast("⚠️ 提示词未能复制，请手动复制后粘贴到网页", false);
					}
				}
    ...
    `);
				...
				function openStoryFullscreen(sc) {
					...
					closeBtn.style.cssText = "position:absolute;top:20px;right:20px;background:rgba(255,255,255,0.1);border:2px solid #594c39;border-radius:8px;color:#e8a33d;padding:8px 16px;cursor:pointer;font-size:14px";
					closeBtn.onclick = () => overlay.remove();
					overlay.appendChild(closeBtn);
					const stage = document.createElement("div");
					stage.style.cssText = "position:relative;width:100%;max-width:1200px;aspect-ratio:16/9;background:#0b0a09;border:3px solid #594c39;border-radius:12px;overflow:hidden";
					overlay.appendChild(stage);
					const grid = document.createElement("div");
					grid.style.cssText = "display:flex;gap:16px;flex-wrap:wrap;justify-content:center;margin-top:20px;max-width:1200px";
					sc.shots.forEach((t, i) => {
						const card = document.createElement("div");
						card.style.cssText = "width:180px;background:#1a1e20;border:2px solid #594c39;border-radius:8px;padding:12px;cursor:pointer";
						...
						if (it && it.url) {
							...
							placeholder.style.cssText = "width:100%;height:100px;background:#2b2e33;border-radius:4px;display:flex;align-items:center;justify-content:center;color:#566;font-size:24px";
							placeholder.textContent = "🖼";
							card.appendChild(placeholder);
						}
						const info = document.createElement("div");
						info.style.cssText = "margin-top:8px;font-size:11px;color:#9aa0a6";
						info.textContent = "#" + (i + 1) + " " + t.speaker;
						card.appendChild(info);
						const text = document.createElement("div");
						text.style.cssText = "margin-top:4px;font-size:10px;color:#efe6d0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap";
						text.textContent = t.text.slice(0, 30);
