					...
					const worker = async () => {
						while (queue.length && !forgeStop) {
							...
							if (ok) {
								...
								err.textContent = "失败:" + line.slice(0, 12) + (lastErr ? " \xB7 " + lastErr.slice(0, 120) : "");
								err.title = lastErr;
